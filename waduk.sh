...
if [[ "$COUNTRY" == "ID" ]]; then
    echo "[INFO] VPS Indonesia terdeteksi"
    echo "[INFO] Install Xray pakai ghproxy (anti setuk)"

    wget -q --show-progress \
    https://ghproxy.com/https://github.com/XTLS/Xray-core/releases/download/v${XRAY_VERSION}/Xray-linux-64.zip

    unzip -o Xray-linux-64.zip
    install -m 755 xray /usr/local/bin/xray

elif [[ "$COUNTRY" == "SG" ]]; then
    echo "[INFO] VPS Singapura terdeteksi"
    echo "[INFO] Install Xray pakai installer resmi"

    bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" \
    @ install -u www-data --version ${XRAY_VERSION}

else
    echo "[INFO] Negara lain ($COUNTRY)"
    echo "[INFO] Pakai metode ghproxy (aman universal)"
...
echo "[INFO] Install Xray selesai"
